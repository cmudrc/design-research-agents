...
class DeterministicSequenceLLMClient:
    ...
    def __init__(self, *, responses: Sequence[str]) -> None:
        """Store a finite response queue consumed in call order."""
    ...
    def chat(
        ...
        *,
        ...
        """Return the next deterministic response as a completed chat result."""
        del messages, params
    ...
    def stream_chat(
        ...
        *,
        ...
        """Emit a deterministic delta event followed by a completed event."""
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        """Adapt request-based invocation to the deterministic chat method."""
        return self.chat(
            request.messages,
    ...
    def stream(self, request: LLMRequest) -> Iterator[LLMDelta]:
        """Yield a single deterministic streaming delta for request-based APIs."""
        response = self.generate(request)
        yield LLMDelta(text_delta=response.text)

    def default_model(self) -> str:
        """Return the stable model identifier used by this deterministic client."""
...
class FixedDesignPeerAgent(Agent):
    ...
    def __init__(self, *, messages: list[str], stop: bool = False) -> None:
        """Initialize fixed delegate outputs for workflow-step examples."""
    ...
    def run(
        ...
        *,
        ...
        """Return fixed contribution payload compatible with delegate batch step outputs."""
...
class EchoDesignReasoningAgent(Agent):
    ...
    def run(
        ...
        *,
        ...
        """Return deterministic recommendation metadata derived from prompt length."""
