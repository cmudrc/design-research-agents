...
from design_research_agents.implementations.agents.direct_llm_call import DirectLLMCall
    ...
    def build_judge_output_from_delegate(
        ...
        if isinstance(maybe_model_response, Mapping):
            with suppress(TypeError):
                self._runtime_state["last_model_response"] = LLMResponse(
                    **dict(maybe_model_response)
                )

        delegate_success = delegate_payload.get("success")
        delegate_output_payload = delegate_payload.get("output")
        delegate_output = (
            dict(delegate_output_payload) if isinstance(delegate_output_payload, Mapping) else {}
        )
