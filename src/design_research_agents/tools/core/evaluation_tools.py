...
def register_evaluation_tools(source: InProcessToolSource) -> None:
    ...
    source.register_tool(
        spec=ToolSpec(
            ...
            description=(
                "Score and rank alternatives against weighted criteria with optional goal-aware "
                "normalization."
            ),
            input_schema={
                "type": "object",
    ...
    source.register_tool(
        spec=ToolSpec(
            ...
            description=(
                "Rank alternatives from pairwise outcomes using Copeland scoring "
                "(wins - losses + 0.5*ties)."
            ),
            input_schema={
                "type": "object",
                "additionalProperties": False,
...
def _parse_criteria(raw_criteria: object) -> list[_Criterion]:
    ...
    return [
        _Criterion(name=item.name, goal=item.goal, weight=item.weight / total_weight)
        for item in parsed
    ]
...
def _criterion_values(*, alternatives: list[_Alternative], criterion_name: str) -> list[float]:
    ...
    for alternative in alternatives:
        if criterion_name not in alternative.scores:
            raise ValueError(
                f"alternative '{alternative.alt_id}' is missing criterion '{criterion_name}'."
            )
