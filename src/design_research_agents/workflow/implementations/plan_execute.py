    ...
    def _run_plan_execute(
        ...
        *,
        ...
        def _executor_prompt_builder(step_context: Mapping[str, object]) -> str:
            ...
            iteration = _parse_loop_iteration(
                raw_iteration,
            ...
            loop_step_records = (
                _normalize_mapping_records(loop_state.get("step_results"))
        ...
        def _state_reducer(
            ...
            state_step_results = _normalize_mapping_records(state.get("step_results"))
            state_final_output = _normalize_mapping(state.get("final_output"))
        ...
        final_state = _normalize_mapping(loop_output.get("final_state"))
        step_results = _normalize_mapping_records(final_state.get("step_results"))
...
def _normalize_mapping(raw_value: object) -> dict[str, object]:
    if not isinstance(raw_value, Mapping):
        return {}
    return dict(raw_value)


def _normalize_mapping_records(raw_value: object) -> list[dict[str, object]]:
...
def _parse_loop_iteration(raw_value: object, *, error_prefix: str) -> int:
