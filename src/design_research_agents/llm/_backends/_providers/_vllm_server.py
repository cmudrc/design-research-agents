    ...
    def _wait_until_ready(self) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if process.poll() is not None:
                raise RuntimeError(
                    "vLLM server exited before becoming ready. Ensure vLLM is installed and server arguments are valid."
                )
        ...
        raise RuntimeError(f"Timed out waiting for vLLM server readiness at {health_url}. Last error: {last_error!r}")
